...
def generateTermMaps():
    ...
    writeDictToFile(termIdMap,"termIdMapStemmerStopWords.txt")
    writeDictToFile(docIdMap,"docIdMapStemmerStopWords.txt")    
...
def readDocumentList():
    ...
    docIdMap = readDictToMemory(docIdMap,"docIdMapStemmerStopWords.txt")
    print len(termIdMap)
    print len(docIdMap)
    for file in listOfFiles:
        i = 0
...
def addDocumentToIndex(docId, corpusContent,termIdMap, docIdMap, invertedListOfTerms):
    file = open("lengthOfDocumentsStemmerStopWords.txt",'a')
    ...
    for stopWord in listOfStopWords:
        if termDict.has_key(stopWord):
            del termDict[stopWord]
...
def writeToFileWithoutStopWords():
    termIdDict = {}
    docIdDict = {}
    listOfStopWords = generateListOfStopWords()
    invertedListFile = open("F:/work/Workspaces/Test/index/invertedList167.txt",'r')
    invertedListFileWithoutStopWords = open("F:/work/Workspaces/Test/index/invertedListStemmerStopWords.txt",'a')
    catalogFile = open("F:/work/Workspaces/Test/index/catalog/catalog167.txt",'r')
    catalogFileWithoutStopWords = open("F:/work/Workspaces/Test/index/catalog/catalogFileStemmerStopWords.txt",'a')
    catalogDict = {}
    termIdDict = readDictToMemory(termIdDict, "termIdMapStemmerStopWords.txt")
    for line in catalogFile:
        catalogSplit = line.split()
        catalogDict[int(catalogSplit[0])] = catalogSplit[1]
    
    for stopWord in listOfStopWords:
        stopWordId = termIdDict.get(stopWord)
        if catalogDict.has_key(stopWordId):
            del catalogDict[stopWordId]
    
    print len(catalogDict)
    
    for key in catalogDict:
        splitTermOffset = catalogDict[key]
        splitOffset = splitTermOffset.split(',')
        invertedListFile.seek(long(splitOffset[0]))
        startOffset = invertedListFileWithoutStopWords.tell()
        writeTerm = invertedListFile.read(long(splitOffset[1]) - 1 - long(splitOffset[0]))
        invertedListFileWithoutStopWords.write(writeTerm)
        endOffset = invertedListFileWithoutStopWords.tell()
        catalogFileWithoutStopWords.write(str(key) + " " + str(startOffset) + "," + str(endOffset) + "\n")
