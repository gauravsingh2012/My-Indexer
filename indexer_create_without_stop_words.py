...
def generateTermMaps():
        ...
        for d in doc:
            ...
            if len(corpusTextContent) > 1:   
                ...
                for termPosition,term in enumerate(terms):
                    termPositionList = []
                    lowerCaseTerms = term.group().lower()                    
    ...
    writeDictToFile(termIdMap,"termIdMap.txt")
    writeDictToFile(docIdMap,"docIdMap.txt")    
...
def readDocumentList():
    ...
    docIdMap = readDictToMemory(docIdMap,"docIdMap.txt")
    print len(termIdMap)
    print len(docIdMap)
    for file in listOfFiles:
        i = 0
...
def writeToFileWithoutStopWords():
    ...
    invertedListFileWithoutStopWords = open("F:/work/Workspaces/Test/index/invertedListWithoutStopWords.txt",'a')
    catalogFile = open("F:/work/Workspaces/Test/index/catalog/catalog167.txt",'r')
    catalogFileWithoutStopWords = open("F:/work/Workspaces/Test/index/catalog/catalogFileWithoutStopWords.txt",'a')
    catalogDict = {}
    termIdDict = readDictToMemory(termIdDict, "termIdMap.txt")
...
def addDocumentToIndex(docId, corpusContent,termIdMap, docIdMap, invertedListOfTerms):
    file = open("lengthOfDocumentsWithoutStopWords.txt",'a')
