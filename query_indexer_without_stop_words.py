...
def queryElasticSearch(queryTermsWithoutStopWords, queryTFDict):
    ...
    invertedListFile = open("F:/work/Workspaces/Test/index/invertedListWithoutStopWords.txt", 'r')
    ...
    catalogMap = readCatalogToMemory(catalogMap, "F:/work/Workspaces/Test/index/catalog/catalogFileWithoutStopWords.txt")
    ...
    for eachTerm in queryTermsWithoutStopWords:
        ...
        for key in termTfMap:
            if (key is not None):
                #okapiTfForAllTerms (float(len(termTfMap.get(key))) , 177.181070687 , float(lengthDict[str(key)]) , str(key), dict)
                #tfIdforAllTerms (float(len(termTfMap.get(key))) , 177.181070687 , float(lengthDict[str(key)]) , str(key) , dict, float(len(termTfMap)))   
                okapiBm25 (float(len(termTfMap.get(key))) , 177.181070687 , float(lengthDict[str(key)]) , str(key) , dict, float(len(termTfMap)), float(queryTFDict[eachTerm1]))
               
        print eachTerm + " Finished"
    #writeQueryModelsToFile(dict, queryNo,"okapiBm25IndexerWithoutStopWords.txt",1000)
    return dict
...
def queryElasticSearchForSmoothing(queryTermsWithoutStopWords, queryTFDict):
    ...
    for eachTerm in queryTermsWithoutStopWords:
        ...
        for key in termTfMap:
            if (key is not None):
                unigramLMLaplaceSmoothing(float(len(termTfMap.get(key))) ,  200826 ,float(lengthDict[str(key)]) , str(key), dict)
    ...
    writeQueryModelsToFile(smoothingDict, queryNo, "laplaceSmoothingMyIndexerWithoutStopWords.txt",1000)
    print queryNo + " Finished"        
...
def queryElasticSearchForJelinek(queryTermsWithoutStopWords, queryTFDict):
    ...
    writeQueryModelsToFile(smoothingDict, queryNo, "jelinekMercerSmoothingWithoutStopWords.txt",1000)
    print queryNo + " Finished"
...
def storeLengthOfDocumentsInDictionary():
    file = open("lengthOfDocumentsWithoutStopWords.txt", "r")
    dict = {}
    for line in file:
...
def proximitySearchModel(queryTermsWithoutStopWords, queryTFDict, combinedDict):
    ...
    writeQueryModelsToFile(combinedDict, queryNo,"proximityWithoutStopWords.txt",100)
    print queryNo + " Finished"
